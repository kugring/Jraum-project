...
const MenuEdit = () => {
    ...
    //          state: 수정할 메뉴 상태             //
    const menu = useMenuPageStore.getState().editMenu!;
    //          state: 메뉴 이미지 상태             //
    const [menuImage, setMenuImage] = useState<string>(menu?.image || '');
    //            state: 이미지 파일 인풋 참조 상태           //
    const imageInputRef = useRef<HTMLInputElement | null>(null);
    //          state: 메뉴 샷 상태           //
    const [shot, setShot] = useState<string>(menu.espressoShot.toString());
    //          state: 메뉴 이름 상태           //
    const [name, setName] = useState<string>(menu.name);
    //          state: 메뉴 가격 상태           //
    const [price, setPrice] = useState<string>(menu.price.toString());
    //          state: 메뉴 활성화 상태             //
    const [status, setStatus] = useState<boolean>(menu.status !== 1);
    //          state: 메뉴 옵션들 상태             //
    const menuOptions: string[] = menu.options.map((menuOption) =>  menuOption.category === "시럽" ? menuOption.detail + "시럽" : menuOption.category)
